...
- `tests/doubles/InMemoryRepository.ts` não reproduz o filtro implícito `deleted_at IS NULL` que `PostgresDataAccess` aplica em toda consulta genérica — ver §11.4 para o risco associado. Ao tocar em qualquer fluxo de soft delete, verificar manualmente contra o banco real antes de confiar no teste unitário isolado.
- Desde a correção do bug de constraint `UNIQUE` em contas removidas, e-mail e username de uma conta com soft delete podem ser reutilizados por um cadastro novo — decisão de negócio, não bug.
