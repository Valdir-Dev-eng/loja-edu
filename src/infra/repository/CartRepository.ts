...
export class CartRepository extends RepositoryPort<Cart> {
    private readonly collectionName = "carrinhos";

    constructor(protected readonly dataAccess: DataAccessPort) {
        super(dataAccess);
    }

    async save(entity: Cart): Promise<string | number | undefined> {
    ...
    async findBy(query: FilterQuery<Cart>): Promise<Cart | null> {
        const data = await this.dataAccess.findOne<Record<string, unknown>>(this.collectionName, this.toColumnQuery(query));
        return data ? this.mapToEntity(data) : null;
    }

    async findByIncludingDeleted(query: FilterQuery<Cart>): Promise<Cart | null> {
    ...
    async findMany(query: FilterQuery<Cart>): Promise<Cart[]> {
    ...
    async decrementFieldIfSufficient(id: string, field: keyof Cart & string, amount: number): Promise<boolean> {
        return this.dataAccess.decrementIfSufficient(this.collectionName, id, field, amount);
    }

    async updateIfEqual(id: string, field: keyof Cart & string, expectedValue: unknown, data: Partial<Cart>): Promise<boolean> {
        return this.dataAccess.updateIfEqual(this.collectionName, id, field, expectedValue, this.toColumnQuery(data));
    }

    async incrementField(id: string, field: keyof Cart & string, amount: number): Promise<void> {
        await this.dataAccess.incrementField(this.collectionName, id, field, amount);
    }

    private toColumns(entity: Cart): Record<string, unknown> {
        return {
            id: entity.id,
    ...
    private toColumnQuery(entity: Partial<Cart>): Record<string, unknown> {
    ...
    private mapToEntity(data: Record<string, unknown>): Cart {
        return new Cart(
            data.id as string,
